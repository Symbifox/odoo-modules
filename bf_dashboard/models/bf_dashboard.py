# -*- coding: utf-8 -*-
"""Agrégateur du tableau de bord Blue Fox.

Aligné sur la garde douce de ``bf_home`` (tâche #24862), pour la même raison :
...
* **Aucune dépendance dure sur les modules BF lus.** ``hosting_management``,
  ``project_knowledge_matrix`` et ``privacy_consent`` sont sondés à l'exécution
  par ``@needs``. Le manifeste ne garde en dur que ``base``, ``account``,
  ``project`` et ``mail`` : les quatre collecteurs comptables interrogent
  ``account_move_line`` en SQL brut, hors de portée de l'ORM et donc de toute
  garde.
...
class BfDashboard(models.AbstractModel):
    ...
    def _get_knowledge_summary(self):
        ...
        # sans le coffre, l'appel levait une AttributeError, `_safe` la
        # rattrapait, et le collecteur tombait EN ENTIER — emportant les deux
        # chiffres de revue, qui eux étaient calculables.
        # Sans le coffre les deux clés valent None, pas zéro : « aucun
        # identifiant n'expire » et « je n'en sais rien » ne se disent pas
        # pareil, et un tableau de bord qui répond zéro rassure à tort.
