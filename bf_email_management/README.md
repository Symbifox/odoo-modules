...
- **Detected from the calendar, when you ask for it (11.35+).** A per-user setting turns a calendar event whose title says holiday, leave or out-of-office into a period for its duration, copied from a default message. It reads the event's **organiser or its attendees**, because on a calendar kept by a synchronisation the organiser is the superuser, not the person who is away; a **declined** invitation arms nothing; an **occurrence of a recurrence** arms nothing, since an annual holiday would otherwise arm a responder every year for centuries; and a duration ceiling (`bf_email.absence_calendar_max_days`, 90 days) bounds a title that lands on a year-long event.
- **No text, no answer.** Without a message of your own and without one lent by another module (`_absence_seed` is the hook), nothing arms: an empty template sent to a client costs more than a silence.
...
- **Reply All from the chatter (11.24+)** — the chatter's « ... » menu offered a plain *Reply*, which addresses the original sender and nobody else; answering a four-person thread meant retyping three addresses or leaving Odoo. **Répondre à tous** puts the sender in *To* and the rest of the thread in *Cc*, reusing the same quoted body, subject de-duplication and signature handling as the single reply. It appears on email messages only, never on an internal note or a system notification.
  - **Odoo does not keep a received email's `To:` and `Cc:` headers.** `_message_route_process` persists only the `partner_ids` it managed to resolve, and it drops the instance's own addresses, so a mail sent to the catchall alone lands with an empty recipient list. Three sources are therefore **added together**, never elected: the `bf.email` mirror (which keeps the headers verbatim), `mail.message.email_to` / `email_cc` when `mail_tracking` is installed, and `partner_ids` / `recipient_cc_ids`. Duplicates are removed per partner afterwards, which is what makes the union safe. Where none of the three knows the other participants, the button degrades to *reply to the sender* rather than inventing recipients.
  - **Addresses that route back into this Odoo are excluded** — the catchall, the bounce alias, the default From and every `mail.alias` — because Cc'ing the gateway makes the reply come back in and re-post itself in the chatter it came from. Your own addresses, including your IMAP accounts' logins and aliases, are excluded too.
  - When the computation finds nobody, the action says so instead of opening an empty composer: a composer with no recipient posts a message that notifies no one, and nothing on screen says it did.
...
- **Auth is a captured web login.** `auth/start` runs at `auth="user"`, so the browser goes through `/web/login` — password, SSO and MFA apply without the app reimplementing any of them. Since 11.36 it no longer emits anything on the GET: it validates the request and shows a **consent page** (account, device, access requested), and only the « Allow » POST to `auth/consent`, CSRF token included and every field re-validated, emits the single-use 3-minute code. Without that step, a third-party app declaring the same URL scheme could pair a device silently in a browser where the person is already signed in. The durable bearer token is only revealed at `auth/exchange`, over HTTPS. **No password route is exposed.**
