...
* **A reader sees a post and a conversation, nothing else.** Audience, expiry, pin, comments and company are the editorial team's controls and appear to that team alone. So does the workflow status bar, along with the record pager, the action cog, the log-note button, the follower list, the attachment box and the thread search. What is left is the post, the comments and the button to write one. Editors keep the full record.
