...
|---------|------|---------|
| 18.0.2.19.0 | 2026-09-21 | A change pulled from another calendar is no longer silent by construction. The pull writes under `no_mail_to_attendees` / `tracking_disable` / `dont_notify`, which was complete silence while the remote server was the one writing to guests; the pull (Nextcloud and Google alike) now also sets the `bf_remote_change` context key, so `bf_calendar_invite` (6.0.0+) can announce a meeting moved in Thunderbird, on a phone or in the web calendar — only when that module's `auto_change_notice` parameter is armed. Soft link: a tenant without `bf_calendar_invite` ignores the key. New read-only probe `bf_ecarts_non_rapatries()` on a Nextcloud sync config: a bounded CalDAV `REPORT` asking only for `etag` and last-modified, which lists what the server holds that Odoo never pulled (missing event, or ETag moved since). It answers the question an Odoo-side check cannot — *did a change happen without Odoo learning of it* — and returns `mesure = False` with the reason when it could not look, so "nothing found" is never confused with "nothing checked" |
| 18.0.2.18.0 | 2026-09-21 | The VEVENT pushed over CalDAV carries a `SEQUENCE` (RFC 5545 §3.8.7.4), taken from `bf_ics_sequence` when `bf_calendar_invite` is installed. Two revisions of the same meeting previously reached the server with neither declaring itself newer, so a client receiving them out of order could not tell which to keep. Without that module nothing is written, rather than a `SEQUENCE:0` that would make a revised meeting pass for the original |
| 18.0.2.17.0 | 2026-09-11 | One `X-` property threw away **every** `VTIMEZONE` of an `.ics`. `dateutil.tz.tzical` raises on any unknown property, and Nextcloud writes `X-TZINFO` in each of its blocks (Google writes `X-LIC-LOCATION`) although RFC 5545 §3.8.8.2 explicitly allows them; the empty table then fell back to reading local wall time as UTC — the same hours-off shift as the Windows timezone names, through another door. Blocks are now sanitised before parsing (RFC 5545 §3.1 unfolding, `X-` properties and lines without `:` dropped), and if batch parsing still fails each `VTIMEZONE` is parsed on its own, so one damaged block no longer takes the valid ones with it. The warning names the offending `TZID` |
| 18.0.2.16.0 | 2026-09-11 | A quoted `ETag` could hold a whole calendar in **full-pull** mode indefinitely. A CalDAV server returns its ETag quoted (RFC 9110 §8.8.3), which is the form any caller reading the `ETag` response header passes back; the pull path parsed `<d:getetag>` and stripped those quotes. Two spellings of the same ETag could therefore never compare equal: the event missed the unchanged-event skip on **every** pass, was rewritten for nothing, and when that rewrite hit a constraint the savepoint rolled the ETag back with it — a loop that never unwound. The ETag is now normalised at **both** ends, at write time and at comparison time, which also heals ETags already stored in their quoted form without a migration. Second and costlier defect: `_store_sync_token()` was only called when the status was `success` or `partial`, so a single permanent error meant no token at all, and a **full** pull every fifteen minutes instead of a delta. The token is now stored in every case — each error being contained by its own savepoint, the token describes a state actually reached — while the status stays `error` so the user-facing notice does not lie |
| 18.0.2.15.0 | 2026-09-05 | The round trip no longer loses the attendees of a booking. Events backed by a `resource.booking` are pushed **without** attendees on purpose, so the Nextcloud scheduling plugin does not send a second iMIP invitation competing with the branded confirmation — but the pull wrote `partner_ids = [(6, 0, …)]`, a replacement built from what the Nextcloud copy carries, that is the calendar owner alone. Every re-ingestion therefore reduced the event to its organiser. Ingestion is now **additive** for those events (Odoo is their source), and the `_bf_odoo_owns_attendees` predicate is the same at both ends so the two halves cannot drift apart again |
| 18.0.2.14.0 | 2026-09-01 | Windows timezone names for Australia, New Zealand and Fiji. A name absent from the map did not fail loudly: resolution returned nothing and the `.ics` parser then read local wall time as UTC, landing an Outlook or Calendly invitation whole hours off |
| 18.0.2.13.0 | 2026-09-01 | Carries the VEVENT `STATUS` both ways. The field itself lives in `bf_calendar_invite`; the link is soft (`in self._fields`), so a tenant without that module is unaffected — the case a "works on my machine" implementation forgets |
