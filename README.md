...
| `bf_appointment` | Symbifox Appointment | 18.0.2.49.0 | BUSL-1.1 | Self-service public booking pages (extends `resource_booking`) |
...
| `bf_cx_subscription` | Expérience client : revenu récurrent à risque | 18.0.1.0.0 | BUSL-1.1 | Recurring revenue at risk on the Customer Experience dashboard tile |
...
| `bf_email_management` | Gestion des courriels | 18.0.9.10.0 | BUSL-1.1 | Unified IMAP inbox + Odoo chatter projection, two-pane OWL folder browser (Apple Mail / Thunderbird), bulk per-row target inference |
| `bf_email_systray` | Boîte de réception bf.email — barre Odoo | 18.0.1.3.0 | LGPL-3 | Systray button opening the bf.email inbox with a (read + unread) counter |
...
| `bf_meeting` | Rencontres | 18.0.3.48.0 | BUSL-1.1 | Agendas, meeting records, and discussion items unified around `calendar.event` with automatic reminders |
...
| `bf_property_core` | Copropriété — Socle | 18.0.1.1.0 | BUSL-1.1 | Quebec condominium associations: buildings, fractions, quote-parts, and a dated register of co-owners that carries undivided ownership |
| `bf_property_finance` | Copropriété — Charges et appels de fonds | 18.0.3.1.0 | BUSL-1.1 | Annual budget and its consultation, the three regimes of common expenses, fund calls allocated to the cent, payments imputed under arts. 1569-1572, interest running from default, arrears and deprivation of the vote, the three contingency-fund floors |
| `bf_property_governance` | Copropriété — Assemblées | 18.0.1.4.1 | BUSL-1.1 | General meetings under the Civil Code: convocation, quorum, weighted votes with their statutory caps and reductions, the three majorities, and a secret ballot that reports what weight exposes |
| `bf_property_loi16` | Copropriété — Loi 16 : carnet, étude et attestation | 18.0.2.0.0 | BUSL-1.1 | Maintenance log, contingency fund study, attestation to a buyer with its printable document, documents owed to a promissory buyer, and the four-regime transitional calendar |
| `bf_property_portal` | Copropriété — Portail de l'occupant | 18.0.3.0.0 | BUSL-1.1 | Announcements, documents, syndicat contact details, maintenance requests and booking of the common spaces, on the Odoo portal, with the audience of each piece enforced by record rules: a tenant is in the register under article 1070 without having access to it, publishing a register item asks the syndicat to acknowledge that it goes beyond article 1070.1, who bears the cost of a request follows the three regimes of article 1064, and a restricted-use space under article 1043 is bookable only by the fractions that hold it |
