...
- **Report follows its project's company, and stays readable on the dark banner** (v18.0.3.62.0): a report whose project belongs to a company takes that company's brand (logo, colours, footer), at creation and when the project is set later, instead of the brand of whoever created it. The "MEETING REPORT" overline on the dark PDF banner is lightened step by step until it reaches 3:1 against the banner (WCAG AA for large text) when the company's primary colour is too dark; the primary colour itself is left untouched everywhere else. `report_accent_on_dark()` is `@api.private`
- **Default report recipients and opt-in automatic sending, per project** (v18.0.3.62.0): a project can carry default report recipients (`meeting_report_recipient_ids`), copied onto its reports while their own recipient list is empty, including when the project is only assigned after the report was created. `meeting_autosend` is an allowlist on the project, unchecked by default: only then may `action_send_report_auto()` send a report without a human click, and it still refuses a report with no recipient or one already sent, and leaves a note in the chatter when it does send
...
- **Public contributions from recipients** — after an agenda in draft has been **sent** and **until it is confirmed**, the email includes a public tokenised link (`/meeting/agenda/<token>`) letting recipients (even without an Odoo account) **propose topics** and **leave comments/notes**. The window opens and closes automatically (`contributions_open ≡ (sent OR pre-opened) AND draft state`); confirming closes the link. Proposed topics arrive in **moderation** (`source='contributed'`, `moderation_state='pending'`) and enter neither the PDF nor the email until a manager accepts them; comments are posted to the chatter and the organiser receives a review activity
- **Contributions can open before the send** (v18.0.3.58.0) — `contributions_preopened` opens the public page without waiting for an agenda email to go out. It exists for modules that hand the link out themselves: the first one is appointment booking, where the agenda is created the moment a meeting is booked and the link rides along in the booking confirmation. At that point the agenda is empty, and that is precisely when "what would you like to discuss?" is worth asking. ⚠️ The flag widens the window only at the **bottom**: it still closes when the agenda is confirmed, and `allow_contributions` still closes it outright. The access token, previously minted at send time, is minted on demand instead — the page still exposes nothing beyond the title, the date, the objectives and the **accepted** topics
- **The contribution page is bilingual** (v18.0.3.58.0) — both it and its "window closed" twin follow the agenda's own language rather than the browser's. The link can arrive inside a booking confirmation rendered in English, and a French page would then contradict the email that brought the reader there
...
| `project.project` (inherited) | "Reports" smart button, default report recipients, automatic report sending allowlist (`meeting_autosend`) |
