...
**103 modules are BUSL-1.1.** The source is published and auditable, and **you
...
| `bf_contact_enrichment` | Enrichissement de contacts | 18.0.1.2.2 | BUSL-1.1 | Business cards (OCR) from the desktop or an installable mobile page, email signatures, vCard import, duplicate detection and completeness scoring, powered by Gen |
...
| `bf_expense_ocr` | Lecture des reçus | 18.0.1.0.0 | BUSL-1.1 | Photograph a meal receipt and let the extraction fill in the total, the taxes and the tip — nothing is pre-filled unless the receipt's own arithmetic balances |
...
| `bf_meeting` | Rencontres | 18.0.3.53.0 | BUSL-1.1 | Agendas, meeting records, and discussion items unified around `calendar.event` with automatic reminders |
...
| `hosting_management` | Gestion d'hébergement | 18.0.2.51.3 | BUSL-1.1 | Manage hosting services, client computer parks (endpoints, BitLocker, Action1 sync) and software license pools |
...
- **103 modules: BUSL-1.1.** Production use for your own internal business
