...
**119 modules are BUSL-1.1.** The source is published and auditable, and **you
...
| `bf_email_management` | Gestion des courriels | 18.0.11.34.0 | BUSL-1.1 | Unified IMAP inbox + Odoo chatter projection, two-pane OWL folder browser (Apple Mail / Thunderbird), bulk per-row target inference |
| `bf_email_systray` | Boîte de réception bf.email — barre Odoo | 18.0.2.1.0 | LGPL-3 | Systray button opening the bf.email inbox with a (read + unread) counter |
...
| `bf_federation` | Fédération | 18.0.1.4.2 | LGPL-3 | Federate two Odoo/Symbifox instances: share an object with the other side, messages and attachments following. The core carries tasks; satellites add deliverables, agendas, process maps and a Discuss channel |
...
- **119 modules: BUSL-1.1.** Production use for your own internal business
