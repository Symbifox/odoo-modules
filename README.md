...
| `bf_babillard` | Babillard | 18.0.1.2.0 | BUSL-1.1 | Announcements with an audience, an expiry date and named read receipts, with a confidential reporting path |
| `bf_babillard_celebrations` | Babillard : célébrations | 18.0.1.0.2 | BUSL-1.1 | A delivered group greeting card appears on the noticeboard |
...
| `bf_babillard_gamification` | Babillard : reconnaissance | 18.0.1.0.3 | BUSL-1.1 | A badge given by a colleague appears on the noticeboard |
...
| `bf_email_management` | Gestion des courriels | 18.0.11.38.0 | BUSL-1.1 | Unified IMAP inbox + Odoo chatter projection, two-pane OWL folder browser (Apple Mail / Thunderbird), bulk per-row target inference |
...
| `bf_scan` | Numériser depuis le téléphone | 18.0.1.1.1 | BUSL-1.1 | An installable phone page with three tiles: a business card becomes a contact, an invoice becomes a draft vendor bill with its photo, and a document goes to the note pad with a reminder or straight into any record's thread |
