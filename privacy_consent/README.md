...
|---------|------|-------------|
| 18.0.4.3.2 | 2026-08 | **Fix, blocking**: every consent mail template opened with a `getattr(...)` expression. `getattr` is not in QWeb's evaluation context, so evaluating it raised a `TypeError` that QWeb wrapped into a `QWebException`: the template did not render at all and **no mail went out**. Seven occurrences across the three data files, replaced with `('company_id' in object._fields and object.company_id) or user.company_id`. Note for existing installs: the three files are `noupdate="1"`, so an upgrade does not rewrite `mail.template` records that already exist. |
| 18.0.4.3.1 | 2026-08 | Security hardening pass across the repo: the consent evidence record stored a client-supplied `X-Forwarded-For` as its IP address, meaning the person consenting chose the value written to their own audit trail. Socket peer only now. |
| 18.0.4.3.0 | 2026-07 | The DocuSeal and LibreSign webhooks read `request.jsonrequest`, removed from Odoo in 17.0; a bare `except` swallowed the `AttributeError`, so every call answered `{"status":"error","message":"Invalid JSON"}` regardless of its payload. Both routes move to `type="http"` with `request.get_json_data()` and answer through `make_json_response`, so the status code carries meaning (400 unparseable, 401 rejected, 200 accepted). |
| 18.0.4.2.0 | 2026-07 | Security: nine missing record rules, so records hanging off a company-scoped parent escaped the boundary protecting the parent itself. `privacy.consent.evidence` had no rule and no `company_id`, leaving the forensic proof (IP address, user agent, and a snapshot naming the data subject) readable across companies while the consent it belongs to was correctly hidden; `privacy.notice.version` leaked the full legal body of every notice. |
