...
**115 modules are BUSL-1.1.** The source is published and auditable, and **you
...
| `bf_appointment` | Symbifox Appointment | 18.0.2.59.0 | BUSL-1.1 | Self-service public booking pages (extends `resource_booking`) |
...
| `bf_appointment_visit` | Symbifox Property Showings | 18.0.1.0.1 | BUSL-1.1 | Showing appointments for real estate: seller availability, visit register, approval loop, occupied dwellings |
...
| `bf_cx` | Expérience client | 18.0.1.8.2 | BUSL-1.1 | Customer experience measurement: NPS programs, continuous feedback, complaints and consent-tracked testimonials |
| `bf_cx_appointment` | Expérience client : feedback post-rendez-vous | 18.0.1.1.1 | BUSL-1.1 | Three-smiley feedback request when an appointment is completed (opt-in) |
...
| `bf_cx_hosting` | Expérience client : CSAT post-maintenance | 18.0.1.1.1 | BUSL-1.1 | CSAT request after a completed planned maintenance (opt-in) |
...
| `bf_cx_meeting` | Expérience client - feedback post-rencontre | 18.0.1.2.1 | BUSL-1.1 | Three-smiley feedback request after a meeting report is sent to the client |
...
| `bf_cx_sign` | Expérience client : feedback post-signature | 18.0.1.1.1 | BUSL-1.1 | Three-smiley feedback request when a signature is completed (opt-in) |
...
| `bf_email_management` | Gestion des courriels | 18.0.11.30.0 | BUSL-1.1 | Unified IMAP inbox + Odoo chatter projection, two-pane OWL folder browser (Apple Mail / Thunderbird), bulk per-row target inference |
...
| `bf_federation` | Fédération | 18.0.1.4.1 | LGPL-3 | Federate two Odoo/Symbifox instances: share an object with the other side, messages and attachments following. The core carries tasks; satellites add deliverables, agendas, process maps and a Discuss channel |
...
| `bf_gantt` | Échéancier | 18.0.1.5.4 | BUSL-1.1 | Gantt schedules for Odoo Community, on a project or standalone, shareable on the portal without a seat, exportable to branded PDF, PNG, SVG, XLSX and MS Project |
...
| `bf_hosting_patch` | Hébergement — Mises à jour du système | 18.0.4.3.0 | LGPL-3 | Fleet update state per installed system — pending packages, kernel, reboots — and applies updates on command via a queue the machines poll |
| `bf_hour_bank` | Banque d'heures | 18.0.1.15.1 | BUSL-1.1 | Automated tracking of client hour banks with threshold-based proactive notifications (unbilled hours, % of allocated budget, balance floor) |
...
| `bf_meeting` | Rencontres | 18.0.3.57.1 | BUSL-1.1 | Agendas, meeting records, and discussion items unified around `calendar.event` with automatic reminders |
...
| `bf_sms_archive` | SMS & Calls | 18.0.5.13.0 | BUSL-1.1 | Two-way live SMS/MMS messaging via VOIP.ms (chat workspace + systray) plus Android SMS/call-log archiving, search, PDF/CSV export and task linking |
...
| `bf_subscription` | Abonnements | 18.0.1.5.0 | BUSL-1.1 | Paid subscription management with correlation to vendor bills |
...
| `bf_universal_search` | BF Recherche universelle | 18.0.2.2.0 | LGPL-3 | Cross-module search through the command palette |
...
| `hosting_management` | Gestion d'hébergement | 18.0.2.51.4 | BUSL-1.1 | Manage hosting services, client computer parks (endpoints, BitLocker, Action1 sync) and software license pools |
...
- **115 modules: BUSL-1.1.** Production use for your own internal business
